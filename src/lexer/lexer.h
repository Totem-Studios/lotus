            ...
            if (isspace((*lexer_ready_content)[index])) {
                ...
                switch ((*lexer_ready_content)[index]) {
                    ...
                    case '/':
                        if ((*lexer_ready_content)[index + 1] == '/') {
                            ...
                            if (index >= lexer_ready_content->size())
                                syntax::error(time, "unclosed multi-line comment", "| encountered unclosed multi-line comment while lexing: ", comment);
                            else
                                index++;
                        } else if ((*lexer_ready_content)[index + 1] == '=') {
                            index++;
                    ...
                    default:
                        ...
                        syntax::/*nonfatal_*/error(time, "unsupported symbol", "| encountered unsupported or undefined character while lexing: ", std::string(1, static_cast<char>((*lexer_ready_content)[index])));
                        break;
                }
            } else {
                // Syntax error!
